...
#### 🏗️ Reference Data UI Infrastructure (Phase 11.1)

##### 📋 Settings Page & Navigation
- **Settings Page**: Новая страница настроек по адресу `/settings`
  - Tab-based навигация для управления справочниками
  - Responsive карточки с иконками для быстрого доступа
  - Breadcrumbs навигация с авто-генерацией из URL
  - Интеграция с основным sidebar меню
  - Секции для импорта/экспорта и безопасности

##### 🔧 Universal CRUD Component
- **CRUDTable.tsx**: Универсальный компонент для управления справочными данными
  - Inline редактирование с валидацией в реальном времени
  - Поиск и фильтрация по всем полям
  - Сортировка по колонкам (asc/desc)
  - Множественный выбор и bulk операции
  - Импорт данных из CSV файлов
  - Экспорт данных в CSV с UTF-8 BOM
  - Диалоги подтверждения удаления
  - Loading и empty states
  - Поддержка различных типов полей (text, number, date, boolean, select)
  - Кастомные render функции для ячеек

##### 📊 Reference Data Managers
- **PeriodManager**: Управление бюджетными периодами
  - Создание периодов с годом и месяцем
  - Валидация уникальности и диапазонов
  - Активация/деактивация периодов
  - Сортировка по дате создания

- **FinancialCenterManager**: Управление финансовыми центрами (ЦФО)
  - CRUD операции для центров финансовой ответственности
  - Управление порядком отображения
  - Статусы активности

- **CostCenterManager**: Управление центрами затрат (МВЗ)
  - CRUD операции для мест возникновения затрат
  - Связь с финансовыми центрами (planned)
  - Управление порядком отображения

- **NomenclatureManager**: Управление номенклатурами
  - Иерархическая структура категорий
  - Типы: доходы (INCOME) и расходы (EXPENSE)
  - Parent-child отношения между категориями
  - Цветовое кодирование по типам

##### 🎨 UI Components
- **Breadcrumbs.tsx**: Компонент навигационных хлебных крошек
  - Автоматическая генерация из URL path
  - Поддержка кастомных items
  - Иконки и separators
  - Локализация путей

- **Radix UI Components**: Добавлены недостающие компоненты
  - `alert-dialog.tsx` - диалоги подтверждения
  - `dialog.tsx` - модальные окна
  - `checkbox.tsx` - чекбоксы для булевых полей

### Технические детали
- **React Hook Integration**: Все компоненты используют современные React hooks
- **TypeScript Generics**: CRUDTable использует generics для type safety
- **API Integration**: Полная интеграция с backend API endpoints
- **Error Handling**: Graceful error handling с toast уведомлениями
- **Performance**: Оптимизированный рендеринг с useMemo и useCallback
- **Accessibility**: ARIA labels и keyboard navigation support
