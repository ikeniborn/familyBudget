...
### 🧠 AI-модуль (в разработке)
- **Свой AI-провайдер** — подключение OpenAI-совместимого API (self-hosted GPU): endpoint, токен и модели настраиваются администратором на странице «🤖 Настройки AI»
- **Три модельных слота** — отдельные модели для текста, изображений и голоса (STT), выбор из живого списка `GET /v1/models`
- **Проверка доступности** — кнопка «Проверить» прогоняет встроенные тест-примеры по каждому слоту
- Дальше по плану: ввод транзакции текстом («кофе 350»), голосом и фото чека, AI-категоризация импорта
