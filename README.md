...
- **Чат по данным аналитики** — на странице аналитики спросите текстом или голосом («Какие затраты по продуктам за текущий месяц?») — сервер сам посчитает агрегаты за нужный период, а ИИ сформулирует ответ строго по этим цифрам; при подборе категорий учитываются их описания. Вопросы и ответы сохраняются на сервере: последние обмены восстанавливаются при открытии страницы, полный список — по кнопке «🕘 История»
