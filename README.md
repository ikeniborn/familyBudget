...
### 🔧 Критическое исправление сериализации API (14.09.2025 - v3.1.1)

✅ **ИСПРАВЛЕНА 500 ОШИБКА** периодов API и улучшена система JSON сериализации:

**Исправление сериализации v3.1.1:**
- **Проблема 500**: Исправлена ошибка JSON сериализации для `/api/periods/` endpoint
- **Pydantic объекты**: Добавлена корректная конвертация в словари через `.dict()`
- **DateTime обработка**: Реализован DateTimeJSONEncoder для автоматической обработки datetime полей
- **Модуль core.response**: Улучшены утилиты `success_response()` и `error_response()`
- **Стабильность API**: Восстановлена полная функциональность CRUD операций с периодами
