...
# t_d_financial_center_data = conn.read(worksheet="t_d_financial_center", usecols=list(range(2)), ttl=5).dropna(how="all")
# # t_d_financial_center_data = t_d_financial_center_data
# print(t_d_financial_center_data)

# t_d_cost_center_data = conn.read(worksheet="t_d_cost_center", usecols=list(range(2)), ttl=5).dropna(how="all")
# # t_d_cost_center_data = t_d_financial_center_data.dropna(how="all")
# print(t_d_cost_center_data)

t_d_accounting_item_data = conn.read(worksheet="t_d_accounting_item_new", usecols=list(range(9)), ttl=5).dropna(how="all")
# t_d_accounting_item_data = t_d_accounting_item_data.dropna(how="all")
filter = t_d_accounting_item_data.set_index(keys='nomenclature')['budget'].filter(like='Баланс')['Баланс']
st.write(filter)
