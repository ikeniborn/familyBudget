...
def form(connection_db:Postgres=None,user_name:str=None):
  ...
  t_d_nomenclature = db_budget.select(sql='select nomenclature_key,nomenclature_name from t_d_nomenclature where is_budget=true',ttl=300)
  ...
  with st.form(key='budget_form',clear_on_submit=True):
    ...
    period_ru_name = st.selectbox('Период',options=t_d_period[(t_d_period['period_dt'] >= function.get_period(-1)) & (t_d_period['period_dt'] <= function.get_period(0))]['period_ru_name'].to_list(),index=1)
    ...
    nomenclature_name = st.selectbox(label='Номенклатура*',options=t_d_nomenclature['nomenclature_name'].to_list(),index=None)
    ...
    if add_row:
      if cost_sum>0 and financial_center_name!=None and nomenclature_name!=None:
        ...
        st.dataframe(data=db_budget.select(sql=
            f'''
            select 
