...
def form(connection_db:Postgres=None):
  ...
  if report_selector:
    ...
    period_ru_name = st.selectbox('Период',options=t_d_period[(t_d_period['period_dt'] >= function.get_period(-1)) & (t_d_period['period_dt'] <= function.get_period(2))]['period_ru_name'].to_list(),index=1)
