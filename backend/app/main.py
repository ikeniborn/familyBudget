...
# Mount PGlite WASM assets (WebAssembly files for offline database)
# Serves postgres.wasm, postgres.data for client-side PGlite database
# Files are generated by Vite during build and placed in frontend/shared/db/assets/
# Only mount if directory exists (may not exist in test environments)
pglite_assets_dir = FrontendPaths.SHARED / "db" / "assets"
if pglite_assets_dir.exists():
    app.mount("/assets", StaticFiles(directory=str(pglite_assets_dir)), name="pglite_assets")
    logger.info(f"Mounted PGlite assets from {pglite_assets_dir}")
else:
    logger.warning(f"PGlite assets directory not found: {pglite_assets_dir} - skipping mount")
