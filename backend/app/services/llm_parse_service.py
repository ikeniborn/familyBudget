...
def _build_batch_prompt(
    ...
    return (
        ...
        "«кофе 350, молоко 100 вчера» — это ДВЕ операции: (кофе, 350, вчера, "
        "категория кафе/еда вне дома) и (молоко, 100, вчера, категория "
        "продукты). Количество объектов в ответе = количеству сумм в тексте.\n"
        "- Общие слова в конце или начале фразы («вчера», «факт», «план», "
        "название счёта) относятся ко ВСЕМ перечисленным операциям, если "
        ...
        "сегодня: «купил», «потратил», «заплатил», слово «факт»); \"plan\" — "
        "будущая или планируемая («заплачу», «планирую», «надо оплатить», "
        "слово «план», будущая дата, следующий месяц). Не ясно — \"fact\".\n"
