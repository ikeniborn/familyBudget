...
Fixes critical SQL error in recalculate_recommended_amounts():
- ERROR: column "fact_type" does not exist in t_f_budget_fact
- Solution: Add INNER JOIN with t_d_article to get type from article
- Part 1 (Global): Replace fact_type with a.type + add JOIN
- Part 2 (Per-category): Replace bf.fact_type with a.type + add JOIN

Root cause:
- t_f_budget_fact does NOT have fact_type column
...
def upgrade() -> None:
    ...
    1. Part 1 (Global recommendations):
       ...
       - Added: AND a.is_current = TRUE (SCD Type 2 filter)
