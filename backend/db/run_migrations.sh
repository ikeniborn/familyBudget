...
LOG_FILE="${SCRIPT_DIR}/migrations.log"
...
is_migration_applied() {
    ...
    local count=$(PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" -t -c \
        "SELECT COUNT(*) FROM ${MIGRATION_TRACKING_TABLE} WHERE migration_file = '${migration_file}'")
...
apply_migration() {
    ...
    if PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" -f "${migration_path}" >> "${LOG_FILE}" 2>&1; then
        ...
        PGPASSWORD="${DB_PASSWORD}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" -c \
            "INSERT INTO ${MIGRATION_TRACKING_TABLE} (migration_file, execution_time_ms, checksum) VALUES ('${migration_file}', ${execution_time}, '${checksum}')" >> "${LOG_FILE}" 2>&1
...
apply_all_migrations() {
    ...
    for file in $(ls "${MIGRATIONS_DIR}"/*.sql | sort -V); do
        ...
        if apply_migration "${basename}"; then
            applied=$((applied + 1))
        else
            failed=$((failed + 1))
