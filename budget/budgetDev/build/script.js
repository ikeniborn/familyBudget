...
function createWebHook() {
  ...
  // variable.boardIdBudget3 = '5e33ed63d17a353274f570ce'
  // variable.boardIdTarget = '5e3cec63b47787407b262e0b'
  // Object.values(variable).forEach((f) => {
  TrelloAPI.createWebHook('5e05161dc3abef51fcf4e761', callbackURL);
  // })
}

function getWebHooksForToken() {
  TrelloAPI.getWebHooksForToken();
}

function deleteWebHooksForToken() {
  TrelloAPI.deleteWebHooksForToken();
}
...
function moveAllCards(postObjectOld, postObjectNew) {
  ...
  try {
    ...
    UrlFetchApp.fetch(
      postObjectOld.apiRoot +
        'lists/' +
        postObjectOld.listId +
        '/moveAllCards?idBoard=' +
        postObjectNew.boardId +
        '&idList=' +
        postObjectNew.listId +
        '&' +
        postObjectOld.keyAndToken,
      data
    );
  } catch (e) {
    addErrorItem(arguments.callee.name + ': ' + e);
