...
Путь внутри бакета настраивается переменной `S3_PATH_PREFIX` в `.env`
(по умолчанию `postgresql-backups`). Бэкапы сохраняются как
`<S3_PATH_PREFIX>/YYYY/MM/backup_*.sql.gz`; `backup.sh`, `restore.sh` и
очистка старых бэкапов используют один и тот же префикс.
