...
sequenceDiagram
    ...
    alt HMAC Valid & auth_date < 86400s
        ...
        Note over API: Generate JWT tokens<br/>access_token (15min)<br/>refresh_token (7d)
...
sequenceDiagram
    ...
    Note over API: Verify password<br/>bcrypt.checkpw(password, hashed_password)
...
### Security Notes
- **bcrypt Hashing**: Password stored with salt rounds = 12
...
sequenceDiagram
    ...
    alt Access Token Valid
        ...
        alt Refresh Token Valid & Not Revoked & Not Expired (7d)
            Note over API: Token Rotation Strategy<br/>1. Generate new access_token (15min)<br/>2. Generate new refresh_token (7d)<br/>3. Revoke old refresh_token
...
|------------|----------|---------|----------|
| Access Token | 15 minutes | localStorage | No rotation |
| Refresh Token | 7 days | localStorage + DB (hashed) | Rotated on refresh |
