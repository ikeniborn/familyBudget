...
**Важно:** Старые транзакции (`t_f_budget_fact`) остаются привязанными к старым версиям категорий. Это позволяет корректно отображать историческую аналитику с правильным типом категории на момент транзакции.
