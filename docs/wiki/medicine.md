...
- **Stock**: `preview_stock` (sync dry-run; warns on missing `expiry_date`) and `execute_stock` find-or-create `t_d_medicine` (active rows only, by name+dosage; unknown `form` falls back to `other` via `VALID_FORMS`; `inn` is imported when present) then insert `t_f_medicine_stock`. Per-row validation includes length caps mirroring the model `max_length`, and the final commit is wrapped — a DB-level failure rolls back and returns a structured error (no partial writes) instead of a 500. `_parse_rows` honors `has_header` (positional `Column_N` keys when false).
