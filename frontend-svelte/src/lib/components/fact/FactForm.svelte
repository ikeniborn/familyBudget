<script lang="ts">
  ...
  function validateForm(): boolean {
    ...
    if (useCostCenter && !formData.cost_center_id) {
  ...
  async function handleSubmit() {
    ...
    try {
      const payload: CreateRegistryData = {
        ...
        cost_center_id: useCostCenter ? parseInt(formData.cost_center_id) : undefined,
      ...
      <form on:submit|preventDefault={handleSubmit} class="space-y-6">
        ...
        <div class="flex items-center space-x-3 p-4 border rounded-md bg-slate-50">
          <input
            type="checkbox"
            id="useCostCenter"
            bind:checked={useCostCenter}
            class="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
          />
          <div class="space-y-1 leading-none">
            <label for="useCostCenter" class="text-sm font-medium text-slate-700 cursor-pointer">
              Использовать МВЗ
            </label>
        ...
        <div class="space-y-2">
          ...
          />
          <p class="text-xs text-slate-500">Введите сумму расхода в рублях</p>
          {#if errors.cost_sum}
