...
export class DexieManager {
  ...
  async createFact(
    ...
    const factToStore: LocalBudgetFact = {
      id: null,
      temp_id,
      ...fact,
      sync_status: 'pending',
  ...
  async updateFact(temp_id: string, updates: Partial<LocalBudgetFact>): Promise<void> {
    ...
    await this.getDB().budgetFacts.where('temp_id').equals(temp_id).modify(updates);
  ...
  async updateRecurringPlan(
    ...
    await this.getDB().recurringPlans.update(id, updates);
