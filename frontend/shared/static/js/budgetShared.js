...
(function (window) {
    ...
    class DateFormatter {
        ...
        static formatForAPIFromMonthName(displayDate) {
            ...
            // Find month index
            const monthIndex = this.RUSSIAN_MONTHS.findIndex(m => m.toLowerCase() === monthName.toLowerCase());
            if (monthIndex === -1)
                return '';
    ...
    `;
      ...
      <div class="p-4">
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="flex items-center gap-2">
            <select class="select select-bordered" data-action="select-month" aria-label="Выбрать месяц">
              ${CalendarWidget.MONTH_NAMES.map((name, i) => `<option value="${i}" ${i === this.currentMonth ? 'selected' : ''}>${name}</option>`).join('')}
            </select>
        ...
        <div class="grid grid-cols-7 gap-1 mb-2">
          ${CalendarWidget.DAY_NAMES.map(day => `<div class="text-center text-xs font-medium text-base-content/60 py-1">${day}</div>`).join('')}
        </div>
    ...
    `;
        ...
        _generateCalendarDays() {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (isToday)
                    btnClass += ' border border-primary';
                if (isSelected)
                    btnClass += ' btn-primary';
                if (isRangeBoundary)
                    btnClass += ' border-2 border-error'; // Red border for range boundaries
                if (isInRange && !isSelected)
                    btnClass += ' bg-range-highlight';
                if (isDisabled)
                    btnClass += ' btn-disabled opacity-30';
                html += `
        <button
          type="button"
          class="${btnClass}"
    ...
    CalendarWidget.MONTH_NAMES = [
        'Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь',
        'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'
    ];
    // Russian day names (short)
    CalendarWidget.DAY_NAMES = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'];
    //=============================================================================
    ...
    class ChoicesCategoryTree {
        ...
        static initializeWorker() {
            if (!this._workerWrapper && typeof window.WorkerWrapper !== 'undefined') {
                try {
                    this._workerWrapper = new window.WorkerWrapper('/static/js/workers/hierarchyWorker.min.js', {
            ...
            const preloadPromises = types.map(async (type) => {
                ...
                if (ChoicesCategoryTree._cache.has(cacheKey)) {
                    if (typeof window.debugLog === "function")
                        window.debugLog(`[ChoicesCategoryTree] ${type} categories already cached, skipping preload`);
                    return;
                }
                // Skip if request already in flight
                if (ChoicesCategoryTree._pendingRequests.has(cacheKey)) {
                    if (typeof window.debugLog === "function")
                        window.debugLog(`[ChoicesCategoryTree] ${type} categories request already in progress, waiting`);
                ...
                try {
                    ...
                    if (typeof window.debugLog === "function")
                        window.debugLog(`[ChoicesCategoryTree] Preloaded ${categories.length} ${type} categories`);
                }
                catch (error) {
        ...
        async loadCategories() {
            ...
            if (!navigator.onLine) {
                if (typeof window.debugLog === "function")
                    window.debugLog('[ChoicesCategoryTree] Offline mode - skipping API call, using cache');
            ...
            const requestPromise = fetch(url, {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 401) {
                        if (typeof window.debugLog === "function")
                            window.debugLog('[ChoicesCategoryTree] User not authenticated - categories not loaded (this is expected for unauthenticated users)');
                ...
                if (staleCache && staleCache.data && staleCache.data.length > 0) {
                    if (typeof window.debugLog === "function")
                        window.debugLog('[ChoicesCategoryTree] Using stale cache for offline mode');
                ...
                if (this.options.financialCenterId) {
                    ...
                    if (allCache && allCache.data && allCache.data.length > 0) {
                        if (typeof window.debugLog === "function")
                            window.debugLog(`[ChoicesCategoryTree] Offline: No cache for FC ${this.options.financialCenterId}, using all categories`);
        ...
        async updateFinancialCenter(financialCenterId) {
            ...
            try {
                ...
                if (this.choices) {
                    ...
                    if (financialCenterId) {
                        if (typeof window.debugLog === "function")
                            window.debugLog(`[ChoicesCategoryTree] Filtered to FC ${financialCenterId}: ${choices.length} categories`);
                    }
                    else {
                        if (typeof window.debugLog === "function")
                            window.debugLog(`[ChoicesCategoryTree] Showing all categories: ${choices.length}`);
                    }
    ...
    ChoicesCategoryTree._cache = new Map();
    ChoicesCategoryTree._pendingRequests = new Map(); // key: "type:showInactive:fcPart" -> Promise
    // Web Worker for hierarchy processing (Phase 2: Performance Optimization)
    ChoicesCategoryTree._workerWrapper = null;
    //=============================================================================
...
export {};
//# sourceMappingURL=budgetShared.js.map