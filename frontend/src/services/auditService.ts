...
class AuditService {
  ...
  async storeDeletedRecord(
    ...
    try {
      ...
      const stored = this.storeDeletedRecord(deletedRecord);
  ...
  async generateAuditReport(
    ...
    try {
      ...
      toast({
        ...
        description: `Отчет "${reportName}" содержит ${total} записей`
      });
  ...
  private storeDeletedRecord(record: Omit<DeletedRecord, 'id'>): DeletedRecord {
    const key = 'audit_deleted_records';
    const stored = localStorage.getItem(key);
    const records: DeletedRecord[] = stored ? JSON.parse(stored) : [];
