...
import { validateReferenceData } from '../utils/validation';
...
// You would create similar services for CostCenter, Nomenclature, and Product
...
export function showBulkOperationResult<T>(result: BulkOperationResult<T>): void {
  ...
  if (errorCount === 0) {
    toast({
      ...
      description: `Обработано: ${totalProcessed}, Успешно: ${successCount}${warningCount > 0 ? `, Предупреждений: ${warningCount}` : ''}`
    });
  } else {
    toast({
      title: 'Операция завершена с ошибками',
      description: `Обработано: ${totalProcessed}, Успешно: ${successCount}, Ошибок: ${errorCount}`,
      variant: 'destructive'
    });
  }
}