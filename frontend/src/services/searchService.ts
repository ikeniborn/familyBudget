...
class SearchService {
    ...
    try {
      switch (sourceEntity) {
        case 'financial_centers':
          // Find cost centers linked to this financial center
          const costCenters = await costCenterService.getAll(userId);
          const linkedCostCenters = costCenters.filter(cc => cc.financial_center_id === sourceId);
          
          if (linkedCostCenters.length > 0) {
            results.push({
              sourceEntity,
              sourceId,
              targetEntity: 'cost_centers',
              targetItems: linkedCostCenters,
              relationship: 'has_cost_centers'
            });
          }
          break;

        case 'cost_centers':
          // Find financial center this cost center belongs to
          const costCenter = await costCenterService.getById(sourceId, userId);
          if (costCenter.financial_center_id) {
            const financialCenter = await financialCenterService.getById(costCenter.financial_center_id, userId);
            results.push({
              sourceEntity,
              sourceId,
              targetEntity: 'financial_centers',
              targetItems: [financialCenter],
              relationship: 'belongs_to_financial_center'
            });
          }
          break;
        ...
        case 'nomenclatures':
          ...
          const nomenclatures = await nomenclatureService.getAll(userId);
          const childCategories = nomenclatures.filter(n => n.parent_id === sourceId);
          ...
          if (childCategories.length > 0) {
            results.push({
              ...
              targetItems: childCategories,
              relationship: 'has_subcategories'
            });
          }

          // Find parent category
          const nomenclature = nomenclatures.find(n => n.nomenclature_id === sourceId);
          if (nomenclature?.parent_id) {
            const parentCategory = nomenclatures.find(n => n.nomenclature_id === nomenclature.parent_id);
            if (parentCategory) {
              results.push({
                sourceEntity,
                sourceId,
                targetEntity: 'nomenclatures',
                targetItems: [parentCategory],
                relationship: 'belongs_to_parent_category'
              });
            }
          }
          break;
        ...
        case 'products':
          ...
          const product = await productService.getById(sourceId, userId);
          if (product.category_ids && product.category_ids.length > 0) {
            const allNomenclatures = await nomenclatureService.getAll(userId);
            const linkedCategories = allNomenclatures.filter(n => 
              product.category_ids!.includes(n.nomenclature_id)
            );
            ...
            if (linkedCategories.length > 0) {
              results.push({
                ...
                sourceId,
                targetEntity: 'nomenclatures',
                targetItems: linkedCategories,
                relationship: 'belongs_to_categories'
              });
            }
          }
          break;
  ...
  private calculateRelevanceScore(item: any, query: string, boost: number): number {
    // This is a simplified relevance scoring algorithm
