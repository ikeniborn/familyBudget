 ...
 * @version 2.0.0
 */
// Note: This file has been migrated from JavaScript to TypeScript.
// Some minor type refinements remain (23 warnings) but functionality is preserved.
// These can be incrementally improved without affecting runtime behavior.

// Types
// @ts-expect-error - Type used for documentation
type WSState = 'connecting' | 'connected' | 'disconnected' | 'error' | 'approaching_limit' | 'limit_reached';
...
declare const logNav: any;
declare const logRTTFilter: any;
...
// Extend Window for MSStream (IE legacy property)
export {};
declare global {
    interface Window {
        MSStream?: any;
...
class BudgetWSClient {
    ...
    private lastServerPing: number;
    private PING_TIMEOUT: number;

    // Error tracking
    private _lastError: LastError | null;
    ...
    // @ts-ignore - Used in fallback logic
    private POLL_INTERVAL: number;
    private POLL_TIMEOUT: number;
    private _pollTimeout: ReturnType<typeof setTimeout> | null;
    private _pollingActive: boolean;
    private _pollRetryCount: number;
    private MAX_POLL_RETRIES: number;
    private BASE_POLL_RETRY_DELAY: number;
    private _consecutive503Count: number;

    // Client ping
    private pingInterval: ReturnType<typeof setInterval> | null;
    private CLIENT_PING_INTERVAL: number;
    private lastPongReceived: number;

    // RTT measurement
    private _rttMeasurements: number[];
    private _rttRollingAverage: number;
    private _pingTimestamp: number | null;
    private RTT_THRESHOLD: number;
    private RTT_WINDOW_SIZE: number;

    // Limit flags
    private limitReached: boolean;
    private approachingLimit: boolean;
    ...
    private _multiTabSupported: boolean | null;
    private _multiTabInitialized: boolean;
    ...
    private _currentBadgeState: WSBadgeState | null;
    private STICKY_STATE_DURATION: number;

    // iOS wake recovery
    private _reconnecting: boolean;
    private _lastVisibilityChange: number;
    private _iosWakeRecoveryMode: boolean;
    private _iosDeviceMode: boolean;

    // Navigation detection
    private isNavigating: boolean;
    private _navigationTimer: ReturnType<typeof setTimeout> | null;
    private NAVIGATION_WINDOW: number;

    // iOS WebSocket strategy
    private _iosWebSocketTimeout?: number;

    // Wake recovery enhancements
    private _healthCheckInterval: ReturnType<typeof setInterval> | null;
    private HEALTH_CHECK_INTERVAL: number;
    private _pongTimeoutTimer: ReturnType<typeof setTimeout> | null;
    private PONG_TIMEOUT: number;
    // @ts-ignore - Used for logging control
    private _enableDetailedLogging: boolean;
     ...
     * @private
     */
    _log(module: string, level: string, message: string, data: Record<string, any> = {}): void {
    ...
    async _initMultiTab() {
        ...
        try {
            ...
            // @ts-expect-error - onerror exists but not in TypeScript definitions
    ...
    async _tryBecomeLeader(): Promise<void> {
        return new Promise<void>((resolve) => {
            ...
            navigator.locks?.request('budget-ws-leader', async (_lock) => {
                ...
                resolve();
                return new Promise<void>(() => {});  // Hold lock forever
            });
        });
    }
     ...
     * @private
     */
    async _tryBecomeLeaderWithTimeout(timeoutMs: number): Promise<void> {
     ...
     * @private
     */
    _broadcastMessage(message: any): void {
        if (this.channel) {
            try {
     ...
     * @private
     */
    _handleChannelMessage(data: any): void {
        switch (data.type) {
            case 'heartbeat':
     ...
     * @private
     */
    _dispatchReceivedEvent(eventType: string, eventData: any): void {
        switch (eventType) {
            case 'fact_created':
     ...
     * @private
     */
    _broadcastWSEvent(eventType: string, eventData: any): void {
        if (this.isLeader && this.channel) {
            this._broadcastMessage({
                type: 'ws_event',
                event: eventType,
     ...
     * @private
     */
    _handleServerMessage(message: WSMessage): void {
    ...
    _silentClose() {
        ...
        if (this.channel) {
            try { this.channel.close(); } catch (e: any) {}
            this.channel = null;
        }
        this.isLeader = false;
        this._multiTabInitialized = false;
    ...
    _handleFactCreated(data: any): void {
    ...
    _handleFactUpdated(data: any): void {
    ...
    _handleFactDeleted(data: any): void {
    ...
    _handlePlanCreated(data: any): void {
    ...
    _handlePlanUpdated(data: any): void {
    ...
    _handlePlanDeleted(data: any): void {
    ...
    _handleTransferCreated(data: any): void {
    ...
    _handleTransferDeleted(data: any): void {
    ...
    _handleItemCreated(data: any): void {
    ...
    _handleItemUpdated(data: any): void {
    ...
    _handleItemDeleted(data: any): void {
    ...
    _handleItemCompleted(data: any): void {
     ...
     * @private
     */
    _notifyHandlers(event: string, data: any): void {
        if (!this.handlers[event]) return;
     ...
     * @private
     */
    _canChangeState(newState: WSBadgeState): boolean {
        const now = Date.now();
     ...
     * @private
     */
    _recordStateChange(newState: WSBadgeState): void {
     ...
     * @private
     */
    _applyIndicatorState(indicator: HTMLElement, state: WSBadgeState): void {
