 ...
 */
export function setupProductAutocomplete() {
        ...
        if (isOnline()) {
            ...
            if (response.ok) {
                ...
                if (state.db && suggestions.length > 0) {
                    await cacheProductSuggestions(suggestions.filter((s) => !s.is_deleted));
                }
            }
        }
        else {
 ...
 */
export function hideProductSuggestions() {
    const dropdown = document.getElementById('product-suggestions-dropdown');
    if (dropdown) {
...
async function cacheProductSuggestions(suggestions) {
    ...
    try {
        ...
        // Get existing cache
        let cached = await state.db.getCache(CACHE_KEY) || [];
        // Merge new suggestions (avoid duplicates by product_name + store_id)
        const existing = new Map(cached.map(s => [`${s.product_name}_${s.store_id}`, s]));
...
async function searchCachedSuggestions(query) {
    ...
    try {
        ...
        const matches = cached.filter(s => s.product_name.toLowerCase().includes(lowerQuery));
