...
export function updateSelectionUI() {
    ...
    if (deleteBtn) {
        if (state.selectedItemIds.size > 0) {
            ...
            if (textSpan && textSpan.classList.contains('hidden')) {
                ...
                if (iconSpan)
                    iconSpan.textContent = `🗑️${state.selectedItemIds.size}`;
            }
            else if (textSpan) {
    ...
    if (selectAllBtn) {
        ...
        if (state.selectedItemIds.size === state.currentItems.length && state.currentItems.length > 0) {
            if (selectAllIconSpan)
                selectAllIconSpan.textContent = '☐';
            if (selectAllTextSpan)
                selectAllTextSpan.textContent = 'Снять выделение';
        }
        else {
            if (selectAllIconSpan)
                selectAllIconSpan.textContent = '☑️';
            if (selectAllTextSpan)
                selectAllTextSpan.textContent = 'Выделить все';
        }
    }
}
// ============================================================================
 ...
 */
export function deleteSelectedWithModal() {
    const state = getState();
    if (state.selectedItemIds.size === 0) {
        return;
    }
