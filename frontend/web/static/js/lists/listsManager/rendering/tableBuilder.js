 ...
 */
export function getProductGroupBreadcrumbs(groupId) {
    if (!groupId)
        return '';
...
export function renderItemsTable() {
    ...
    if (filteredItems.length === 0) {
        ...
        if (desktopTable)
            desktopTable.classList.add('table-content-hidden');
        if (emptyState)
        ...
        if (state.searchQuery && state.searchQuery.trim() !== '') {
            ...
            if (emptyText)
                emptyText.textContent = 'Попробуйте изменить поисковый запрос';
            if (emptyButton) {
            ...
            if (emptyTitle)
                emptyTitle.textContent = 'Все товары выполнены';
            if (emptyText)
                emptyText.textContent = 'Нажмите "Показать все" чтобы увидеть выполненные товары';
            if (emptyButton) {
            ...
            if (emptyText)
                emptyText.textContent = 'Добавьте первый товар или импортируйте из CSV';
            if (emptyButton) {
                emptyButton.textContent = '➕ Добавить товар';
                emptyButton.onclick = () => openAddItemModal();
            }
    ...
    if (desktopTable)
        desktopTable.classList.remove('table-content-hidden');
    if (emptyState)
