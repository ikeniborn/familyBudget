 ...
 */
export async function handleCreateList(event) {
...
export function openEditItemModal(itemId) {
    ...
    if (modalTitle)
        modalTitle.textContent = '✏️ Редактировать товар';
...
export function closeItemModal() {
    ...
    if (hierarchyView?.swipeHandler) {
        ...
        if (swipeHandler.modalOpenedBySwipe) {
            ...
            const swipedElement = document.querySelector(`.hierarchy-item[data-item-id="${itemId}"]`);
            if (swipedElement) {
 ...
 */
export async function handleDeleteFromModal() {
    const itemIdInput = document.getElementById('item-id');
    if (!itemIdInput)
        return;
 ...
 */
export async function handleSaveItem(event) {
    event.preventDefault();
    const form = event.target;
    const formData = new FormData(form);
 ...
 */
export function openDeleteListModal(listId, listName) {
