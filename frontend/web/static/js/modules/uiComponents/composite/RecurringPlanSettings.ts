...
export interface RecurringPlanSettingsProps {
  name: string;
  frequencyType?: 'weekly' | 'monthly' | 'yearly';
  frequencyValue?: number;
  endDate?: string | null;
  className?: string;
  onChange?: (data: RecurringSettings) => void;
}
...
export class RecurringPlanSettings {
  ...
  private frequencyTypeSelect: SelectDropdown;
  private frequencyValueInput: AmountInput;
  private endDateInput: DateInput;
  private helpText: HTMLDivElement | null = null;

  constructor(private props: RecurringPlanSettingsProps) {
    ...
    this.frequencyTypeSelect = new SelectDropdown({
      ...
      onChange: (value) => this.handleFrequencyTypeChange(value as string)
    });
   ...
   */
  private getMaxFrequencyValue(type: string): number {
    switch (type) {
      case 'weekly':
   ...
   */
  private handleFrequencyTypeChange(type: string): void {
   ...
   */
  private updateHelpText(type: string): void {
  ...
  validate(): ValidationResult {
    ...
    const type = this.frequencyTypeSelect.getValue() as string;
   ...
   * Get current settings
   */
  getValue(): RecurringSettings {
    const endDateValue = this.endDateInput.getValue();
    return {
      frequencyType: this.frequencyTypeSelect.getValue() as 'weekly' | 'monthly' | 'yearly',
   ...
   */
  setValue(data: Partial<RecurringSettings>): void {
