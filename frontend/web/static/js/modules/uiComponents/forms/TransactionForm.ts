...
export interface TransactionFormProps {
  ...
  onSubmit: (data: TransactionData) => Promise<any>;
  onSuccess?: () => void;
...
interface FactHints {
  period_plan: number | null;
  period_fact: number | null;
}
...
export class TransactionForm {
  ...
  // UI elements
  private factHintsContainer: HTMLDivElement | null = null;
  ...
  private createRecordTypeField(): HTMLDivElement {
    ...
    expenseLabel.className = `btn btn-sm btn-outline btn-error transaction-type-btn ${
    ...
    incomeLabel.className = `btn btn-sm btn-outline btn-success transaction-type-btn ${
   ...
   */
  private createFactHintsContainer(): HTMLDivElement {
   ...
   */
  private handleFinancialCenterChange(fcId: number | null): void {
   ...
   */
  private handleArticleChange(_article: any): void {
    this.updateFactHints();
  }
   ...
   */
  private async updateFactHints(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/analytics/fact-hints?date=${date}&financial_center_id=${fcId}&article_id=${articleId}`
      );
      ...
      const hints: FactHints = await response.json();
   ...
   * Display fact hints
   */
  private displayFactHints(hints: FactHints): void {
    if (!this.factHintsContainer) return;
   ...
   * Collect form data
   */
  private collectFormData(): TransactionData {
