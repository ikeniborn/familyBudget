...
export interface TransferFormProps {
  ...
  transferType: 'fact' | 'plan';
  initialData?: TransferData;
  onSubmit: (data: TransferData) => Promise<any>;
  onSuccess?: () => void;
  onError?: (error: Error) => void;
}

export interface TransferData {
  transfer_type: 'fact' | 'plan';
  transfer_date?: string;              // For fact transfers
  transfer_plan_month?: string;        // For plan transfers
  from_financial_center_id: number;
  from_article_id: number;             // debit category
  from_cost_center_id?: number | null;
  to_financial_center_id: number;
  to_article_id: number;               // credit category
...
export class TransferForm {
  ...
  // Components
  private dateInput: DateInput | null = null;
  private fromFcSelect: FinancialCenterSelect;
  private fromArticleSelect: ArticleSelect;
  private fromCcSelect: CostCenterSelect;
  private toFcSelect: FinancialCenterSelect;
  private toArticleSelect: ArticleSelect;
  private toCcSelect: CostCenterSelect;
  ...
  constructor(private props: TransferFormProps) {
  ...
  private collectFormData(): TransferData {
    ...
    if (this.props.transferType === 'fact' && this.dateInput) {
      data.transfer_date = this.dateInput.getValue() as string;
    }
