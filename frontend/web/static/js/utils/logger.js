...
class Logger {
     ...
     * @param {string} prefix - Log prefix (e.g., '[API]', '[DB]', '[SYNC]')
     * @param {string} moduleKey - Module key for configuration (e.g., 'API', 'DB', 'SYNC')
     */
     ...
     */
    get isEnabled() {
        return window.LOGGING_CONFIG?.enabled &&
               window.LOGGING_CONFIG?.modules[this.moduleKey];
    }
    ...
    isLevelEnabled(level) {
        return this.isEnabled && window.LOGGING_CONFIG?.levels[level];
    }
