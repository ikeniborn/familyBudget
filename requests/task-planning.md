...
## Задачи
https://budget-dev.ikeniborn.ru/webapp
1. Изменить расположение кнопок на злавной странице
   1. Расположить каждую кнопку отдельно, горизонтально для узкого экрана. к примеру телефона 
   2. Каждая кнопка идет ниже другой для узкого экрана
      1. Сегодня
      2. Статисика
      3. Планфакт
      4. Список
2. ПРи открытиии добавит ьтранзакцию ошибка инициализациии в логах 
app.min.js:1 App: Initialization error ReferenceError: BudgetShared is not defined
    at setupDateInput (add.html:529:13)
    at window.pageInit (add.html:312:13)
    at BudgetApp.init (app.min.js:1:753)
    at async HTMLDocument.<anonymous> (app.min.js:1:1567)
init	@	app.min.js?v=20251120_1308:1
await in init		
(anonymous)	@	app.min.js?v=20251120_1308:1
3. Для большого экрана изменить grid на 4 на строке
4. Из бота удалить исползование команд типа /add, /stats. Оставить тоолько /start
5. При открытии формы добавить транзаци в поле катеоория пелйсхолжер "Загрузка категории..." не пропадает после загрузки
6. Главная страница вебаппп. в правом верхнем углу есть срелка назад, но уже некуда возвразаться. Для главной страницы вебаппп эту кнопку убратть.
...
### ⚠️ КРИТИЧЕСКИ ВАЖНО: ТОЧКА ОСТАНОВКИ

**ЭТО НЕ ФАЗА ВЫПОЛНЕНИЯ!**

Следующий шаг - ЗАВЕРШЕНИЕ ПРОЦЕССА ПЛАНИРОВАНИЯ.
НЕ путать с "Phase 1" создаваемого плана - это разные вещи!

**Что происходит дальше:**
- ✓ Выводится финальное сообщение
- ✓ Работа ПОЛНОСТЬЮ ОСТАНАВЛИВАЕТСЯ
- ✗ НИКАКОГО выполнения фаз плана
- ✗ НИКАКОГО перехода к task-execution

---

### FINAL STEP: ЗАВЕРШЕНИЕ ПЛАНИРОВАНИЯ
...
ПЕРЕД финальным шагом ОБЯЗАТЕЛЬНО:
...
#### **Шаг 8. [MANDATORY] Верификация созданных файлов**

**Validation:** `critical`
**Blocking:** `true`

**Действия:**
1. Проверить существование `plans/` директории
2. Проверить существование `plans/master-plan.md`
3. Проверить существование всех `plans/phase-{N}-{slug}.md` файлов
4. Подсчитать количество файлов (должно быть = total_phases + 1)

**Mandatory Output:**
```
✅ ВЕРИФИКАЦИЯ ФАЙЛОВ ПЛАНА

Директория: plans/ ✓
Master Plan: plans/master-plan.md ✓
Phase Files: {N} файлов ✓
  ✓ plans/phase-1-{slug}.md
  ✓ plans/phase-2-{slug}.md
  ...

ВСЕГО ФАЙЛОВ: {N+1} (ожидалось: {total_phases + 1})
СТАТУС: ✓ ALL FILES VERIFIED
```
...
**Violation Action:**
- Файл отсутствует → FATAL STOP, вернуться к Шагу 6
- Количество файлов некорректно → FATAL STOP, проверить Шаг 6
...
⚠️ РЕЖИМ: ПЛАНИРОВАНИЕ (ЗАВЕРШЕН)
⚠️ ВЫПОЛНЕНИЕ ФАЗ: НЕ НАЧАТО
⚠️ СЛЕДУЮЩЕЕ ДЕЙСТВИЕ: ОСТАНОВКА РАБОТЫ

ЭТО ШАБЛОН ПЛАНИРОВАНИЯ - НЕ ВЫПОЛНЕНИЯ!

❌ НЕ выполнять Phase 1 автоматически
❌ НЕ переходить к task-execution-template.md
❌ НЕ начинать никакие действия из плана
❌ НЕ создавать git ветку
❌ НЕ изменять файлы кода

✅ ТОЛЬКО: Вывести это сообщение и ОСТАНОВИТЬСЯ

═══════════════════════════════════════════════════════════

📋 ЧТО ДАЛЬШЕ (для пользователя):

Планы созданы и готовы к выполнению.
Для выполнения каждой фазы используйте ОТДЕЛЬНЫЙ запрос:

Команда для выполнения фазы 1:
"Выполни фазу: plans/phase-1-{slug}.md используя task-execution-template.md"

⚠️ Каждая фаза выполняется отдельным запросом!

═══════════════════════════════════════════════════════════

🛑 РАБОТА ЗАВЕРШЕНА. ОСТАНАВЛИВАЮСЬ ЗДЕСЬ.

═══════════════════════════════════════════════════════════
```

**Exit Conditions (CRITICAL):**
- ✓ Финальное сообщение выведено ПОЛНОСТЬЮ
- ✓ Работа НЕМЕДЛЕННО ОСТАНОВЛЕНА
- ✓ НЕ выполнено ни одного действия из плана
- ✓ НЕ создано никаких файлов кроме plans/
- ✓ НЕ вызван task-execution-template.md
- ✓ НЕ начато выполнение Phase 1

**MANDATORY STOP:**
После вывода финального сообщения:
- НЕМЕДЛЕННАЯ остановка всей работы
- НЕ интерпретировать plan files как команды к выполнению
- ЖДАТЬ нового явного запроса от пользователя

**Violation Action:**
Если обнаружено выполнение фаз после этого сообщения:
- ОШИБКА: Нарушен режим планирования
- Шаблон использован некорректно
...
12. ✓ CHECKPOINT: STRUCTURED OUTPUT + Markdown (Phase 0)
13. ✓ ⚠️ ТОЧКА ОСТАНОВКИ: Проверка режима
14. ✓ Шаг 8: MANDATORY - Верификация созданных файлов
15. ✓ FINAL STEP: Финальное сообщение
16. ✓ **НЕМЕДЛЕННАЯ ОСТАНОВКА** - НЕ выполнять фазы плана!
...
- После Шага 16 - MANDATORY STOP, никаких действий
- НЕ начинать выполнение Phase 1 из плана
- НЕ переходить к task-execution-template.md
...
- ✓ Проверяйте существование файлов (Шаг 8)
- ✓ ОСТАНАВЛИВАЙТЕСЬ после финального сообщения
...
- ❌ НЕ начинайте выполнение фаз плана (это ПЛАНИРОВАНИЕ!)
- ❌ НЕ игнорируйте exit_conditions
- ❌ НЕ игнорируйте entry_conditions
- ❌ НЕ сокращайте обязательные outputs
- ❌ НЕ путайте "FINAL STEP" с "Phase 1 плана"
- ❌ НЕ выполняйте действия из созданных планов
- ❌ НЕ создавайте git ветки
- ❌ НЕ изменяйте код
- ❌ НЕ вызывайте task-execution-template.md автоматически
