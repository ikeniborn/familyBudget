...
Изучи тщательно текущее решение по SSE. сейчас при каждом переклчюение в рамках одного устровйсва и польвоателя создается новое соединение.
что требуетсяпостоянного переклчюения и ожидания. 
Изучи официалльную документацию и сформируй решение по оптимизации с переиспользвоанием подклчюения для одного устройсва для современных браузеров в том числе safary. 
Решение должно оптимально переиспользвоать созданное подклчюение а так же держать подключение в бэкграунден для рабоыт пуш уведомлений.
Но не должно создавать избыточных соединений в рамкх одного устройсва.
