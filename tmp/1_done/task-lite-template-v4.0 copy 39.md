...
Для модальных форма добавления факта по расходам и доходам реализовать отображение информации по запланированным и фактических отраженных в учете сумм в рамках месяца на основании указанной даты по цфо и категории.
Получать все планы, агрегировать их в рамках полного месяца, цфо, категории.
Данные оторажать по аналогии как это реализаваео для планового модельного окна 
<div id="plan-hints-container" class="grid grid-cols-2 gap-1">
                <button type="button" class="btn btn-xs btn-ghost btn-disabled" id="hint-prev-plan" disabled="">
                    План пред. мес: --
                </button>
                <button type="button" class="btn btn-xs btn-ghost btn-disabled" id="hint-prev-fact" disabled="">
                    Факт пред. мес: --
                </button>
            </div>
Только кнопки не должны быть активными а только отображать запланированные суммы и уже доабвленные фактические данные. 
Если данны нет то отображать серым цветом плашку. 
