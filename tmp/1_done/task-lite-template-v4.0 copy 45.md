...
https://budget.ikeniborn.ru/lists
Изучи id="item-modal"
Изучи решение для подтрочного поиска по товарам из бд 
<input type="text" name="product_name" id="item-product-name" class="input input-bordered input-sm sm:input-md" placeholder="Молоко 3.2%" required="" autocomplete="off" oninput="window.listsManager?.handleProductInput(this.value)">
Запросы отправлятся, но никаких данных не возвращается 
