...
Проанализирвоать текущую реализацию SSE. При запуске на safarу iphone бейдж всегда серым цветом. 
При открытии в десмктопной версии такоих проблем нет. 
ПРи переклчюениях между вкладваки права постоянно происходит пересоздание подключение. В архитектурном решение фиксировалось переиспользование между переключениями. Этот варинат тоже проверитью 
