...
https://budget-dev.ikeniborn.ru/lists
Для всех версий просмотра списа товара добавить кнопку "Скрыть выполненные".
Кнопка доступна для всех версий мобильной и десктопной. 
ПРи нажатии скрывает все выполненны и менетяс на "показать все". 
Учитывать размеры экрана. 
Разместить для удобства выбора. 
Кнопку <button class="btn btn-sm btn-error" onclick="deleteCompleted()" title="Удалить выполненные">
                    <span>🗑️</span><span class="hidden sm:inline ml-1">Удалить выполненные</span>
                </button> прятать пока нет выделенных товаров.
