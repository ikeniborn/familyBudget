...
Добавить статус в субд, апи и фронт о том что записи добавлены после офланй синхронизации.
ВЫводить информацию в 
https://budget-dev.ikeniborn.ru/ id="recent-transactions" 
Реалзиовтаь через статус. Если загружено через офланй то истина, если через онлайн то ложь.
ПРизнак разместить после <th>Описание</th>

1. Подключись по ssh budget-test
2. Выполни в каталоге ~/familybudget git pull в ветке test
3. Выполни в каталоге скрпит sudo bash deploy.sh --sync-mode update --cleanup-mode smart --patch
4. Проанализируй логи терминала процесса установки. После успешлого завершения проанализируй результаты /opt/budget/logs/deploy.log
5. Проанализируй логи запущенных контейнеров /opt/budget + docker compose
6. Исправь все выявленные проблемы локально в скрпитах и сделй коммит и пуш в ветку test
7. По завершению деплоя проверяй на не завершенные процессы на сервере. Не оставляй лишних процессов запущенных в процессе деплоя.
