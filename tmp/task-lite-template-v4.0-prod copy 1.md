...
1. Подключись по ssh budget-prod
2. Выполни в каталоге ~/familybudget git pull в ветке prod
3. Выполни в каталоге скрпит sudo bash deploy.sh --sync-mode update --cleanup-mode smart --major
...
5. Запусти и проанализируй ~/familybudget sudo bash logs.sh
6. Проанализируй логи запущенных контейнеров /opt/budget + docker compose
7. Исправь все выявленные проблемы локально в скрпитах и сделй коммит и пуш в ветку test
8. По завершению деплоя проверяй на не завершенные процессы на сервере. Не оставляй лишних процессов запущенных в процессе деплоя.
