...
def _generate_hash(data: dict, token: str) -> str:
    ...
    data_check_string = "\n".join(f"{key}={value}" for key, value in request_data_alph_sorted)

    secret_key = hashlib.sha256(token.encode()).digest()
    generated_hash = hmac.new(key=secret_key, msg=data_check_string.encode(), digestmod=hashlib.sha256).hexdigest()
